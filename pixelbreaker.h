...
#define IS_TYPING() (timer_elapsed_fast(tap_timer) < TAPPING_TERM * 3)
...
// clang-format off
/*
#if defined(KEYBOARD_hummingbird)
    // specifically for hummingbird's odd matrix
    #define IS_LEFT(e) (e.key.col < 2 || (e.key.col == 2 && e.key.row
       % 2 == 1))
  ...
  #elif defined(KEYBOARD_charybdis)
    // specifically for charybdis's matrix
    ...
    #define IS_UNILATERAL_TAP(r, n) (IS_HOMEROW(r) && (r->event.key.col != n.event.key.col) && (    \
        (r->event.key.row == 1 && 0 <= n.event.key.row && n.event.key.row <= 2) || \
        (r->event.key.row == 5 && 3 <= n.event.key.row && n.event.key.row <= 6) ))

    // Mod-tap and the key that follows are on opposite sides of the keyboard
    #define IS_BILATERAL_TAP(r, n) ( \
        (r->event.key.row == 1 && 3 <= n.event.key.row && n.event.key.row <= 6) || \
